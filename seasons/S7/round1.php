...
<div class="pageTitle">Season 7 Round 1</div>
  ...
  <?php 
    ...
    $pdo = connectToDB();
      ...
      $query = "SELECT t1.name AS team1_name, t1.logo AS team1_logo, t2.name AS team2_name, t2.logo AS team2_logo, m.Team1score, m.Team2score, m.time, m.date
                FROM matches m
                JOIN teams t1 ON m.team1 = t1.id
                JOIN teams t2 ON m.team2 = t2.id
                WHERE m.season = 7 AND m.round = 1
    ...
    if ($statement) {
      ...
      echo '</div>';
      echo '</div>';
      echo '<div class="matchDate">' .$row["time"] . '</div>';
      echo '<div class="matchDate">'. $row["date"] . '</div>';
